...
The light environment does not include `torch` or `transformers`. In that case `AICCompetitionEngine` uses a deterministic hashing text encoder so tests and UI remain usable offline. This fallback is not a real CLIP text-image retrieval model.
