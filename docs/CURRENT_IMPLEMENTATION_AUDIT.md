...
| Metadata fields | Media title/description/tags duoc gop thanh `media_text` roi nap vao `llm_caption`. | Metadata bi nham voi frame caption va co the double-count/khong truy vet duoc. | `aic2026/dataset.py`, `ingestion/schemas.py` | Phase 3/9: tach `media_title`, `media_description`, `media_tags`, `frame_caption`, `ocr_text`, `asr_text`. |
...
13. Loader silently truncates when map rows and feature vectors differ.
