...
| Dataset validation | Fixed in Phase 3: map/frame, mmap feature, keyframe, object/media JSON and source inventory checks produce atomic structured reports. | Builds are gated by `valid_for_index_build`; real data inspection is currently invalid because required keyframes/frame IDs fail checks. | `aic2026/dataset_validation.py`, `aic2026/cli.py`, `aic2026/engine.py` | Complete in code; source data must be repaired before real cache rebuild. |
